...
{
  qDebug() << "adding group: " << group.getName() << " gid: " << group.getGID();
  mAdd.append( group );
}

void KU_Groups::del(int index)
{
  qDebug() << "deleting group: " << at(index).getName() << " gid: " << at(index).getGID();
  mDel.append( index );
}

void KU_Groups::mod(int index, const KU_Group &newgroup)
{
  qDebug() << "modify group " << newgroup.getName() << " gid: " << newgroup.getGID();
  mMod.insert( index, newgroup );
}

void KU_Groups::commit()
{
  qDebug() << "KU_Groups::commit()";
